...
const api = {
  ...
  createBorrowing: async (token, borrowData) => {
    const res = await fetch(`${API_URL}/borrowings`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
      ...
      <div className="flex-1 overflow-auto">
        <div className="p-8">
          ...
          {activeTab === 'members' && (
            <div>
              <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold text-gray-800">A'zolar</h1>
                <button
                ...
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">Olingan kitoblar</th>
          ...
          {activeTab === 'borrowings' && (
            <div>
              <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold text-gray-800">Ijaralar</h1>
                <button
                  onClick={() => handleAddNew('borrowing')}
                  className="flex items-center gap-2 bg-indigo-600 text-white px-6 py-3 rounded-lg hover:bg-indigo-700 transition-colors"
                >
                  <Plus className="w-5 h-5" />
                  Yangi ijara
